...
const Matches = () => {
  ...
  useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_BACKEND_URL}/matches/${series}`)
      .then((response) => {
  ...
  return (
    <div className='w-full h-full'>
      <section className='w-full h-full flex flex-col justify-center bg-[#F2F3F4]'>
        {match.sort((a,b) => new Date(a.match?.dates[0]?.date) - new Date(b.match?.dates[0]?.date)).map((item) => {
          return (
            <div className='w-[65%] h-40 cursor-pointer rounded-[30px] mx-24 shadow-[0_10px_25px_rgba(0,0,0,0.1)] border-2 my-4'
